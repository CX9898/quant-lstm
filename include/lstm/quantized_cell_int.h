...
inline std::int64_t centeredValue(std::int32_t value,
...
inline std::int64_t checkedProduct(std::int64_t lhs, std::int64_t rhs,
                                   const char* message) {
...
inline std::int32_t addZeroPointAndClamp(std::int64_t centered,
                                         const QuantizedPoint& target) {
    target.param.validate(target.type);
    const __int128 translated =
        static_cast<__int128>(centered) + target.param.zero_point;
    ...
    return quantization::clampToRange(static_cast<std::int64_t>(translated),
                                      target.type);
}
...
inline QuantizedCellIntResult computeQuantizedCellInt(
    std::int32_t forget_gate, std::int32_t old_cell,
    std::int32_t input_gate, std::int32_t cell_gate,
    const CellExecutionParams& params) {
    QuantizedCellIntResult result;
    const std::int64_t centered_forget =
        detail::centeredValue(forget_gate, params.forget_gate);
    const std::int64_t centered_old_cell =
    ...
    const std::int64_t centered_cell_gate =
        detail::centeredValue(cell_gate, params.cell_gate);
    result.diagnostics.p_forget =
        detail::checkedProduct(centered_forget, centered_old_cell,
                               "Cell forget 乘积溢出 int64");
    result.diagnostics.p_input =
        detail::checkedProduct(centered_input, centered_cell_gate,
                               "Cell input 乘积溢出 int64");
    if (params.forget_scale.multiplier < 0 ||
    ...
    result.diagnostics.scaled_forget_contribution =
        static_cast<__int128>(result.diagnostics.p_forget) *
        params.forget_scale.multiplier;
    result.diagnostics.scaled_input_contribution =
        static_cast<__int128>(result.diagnostics.p_input) *
        params.input_scale.multiplier;
    result.diagnostics.pre_round_sum =
        result.diagnostics.scaled_forget_contribution +
        result.diagnostics.scaled_input_contribution;
    result.diagnostics.forget_contribution_vanished =
        result.diagnostics.p_forget != 0 &&
        params.forget_scale.multiplier == 0;
    result.diagnostics.input_contribution_vanished =
        result.diagnostics.p_input != 0 &&
        params.input_scale.multiplier == 0;

    const __int128 rounded = quantization::roundShiftRight128(
        result.diagnostics.pre_round_sum, Q31Scale::kFractionalBits);
    const std::int64_t centered =
        quantization::checkedInt128ToInt64(rounded);
    result.value =
...
inline QuantizedHiddenIntResult computeQuantizedHiddenInt(
    std::int32_t output_gate, std::int32_t cell_tanh,
