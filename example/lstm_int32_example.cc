...
std::vector<std::int32_t> quantizeTensor(
    const std::vector<float>& source, quant_lstm::QuantOperator id,
    ...
    for (std::size_t index = 0; index < source.size(); ++index) {
        ...
        result[index] = quant_lstm::quantization::quantize(
            source[index], operator_params.values.at(parameter_index),
            config.at(id).type);
    }
    return result;
}
...
int main() {
    try {
        ...
        quant_lstm::LstmCalibrationSession calibration(
            config, shape.input_size, shape.hidden_size, true);
        calibration.collect(
            shape, weights, input.data(), nullptr, nullptr);
        const auto& finalized = calibration.finalize();

        const auto q_input = quantizeTensor(
            input, QuantOperator::Input, config,
            finalized.quant_params);
        const auto q_weight_ih = quantizeTensor(
            weight_ih, QuantOperator::WeightInputHidden, config,
            finalized.quant_params,
            static_cast<std::size_t>(shape.input_size));
        const auto q_weight_hh = quantizeTensor(
            weight_hh, QuantOperator::WeightHiddenHidden, config,
            finalized.quant_params,
            static_cast<std::size_t>(shape.hidden_size));
        const auto q_bias_ih = quantizeTensor(
            bias_ih, QuantOperator::BiasInputHidden, config,
            finalized.quant_params, 1);
        const auto q_bias_hh = quantizeTensor(
            bias_hh, QuantOperator::BiasHiddenHidden, config,
            finalized.quant_params, 1);

        const std::size_t output_count = static_cast<std::size_t>(
            shape.sequence_length * shape.batch_size *
            shape.hidden_size);
        ...
        quant_lstm::lstmForwardInt32CpuReference(
            ...
            {q_weight_ih.data(), q_weight_hh.data(),
             q_bias_ih.data(), q_bias_hh.data()},
            q_input.data(), nullptr, nullptr, config,
            finalized.quant_params, finalized.execution_params,
            q_output.data(), q_hidden.data(), q_cell.data());

        const auto& output_params =
            finalized.quant_params.at(QuantOperator::Output);
        std::cout << std::fixed << std::setprecision(6);
        for (std::int64_t time = 0;
             time < shape.sequence_length; ++time) {
            std::cout << "t=" << time << ':';
            for (std::int64_t hidden = 0;
                 hidden < shape.hidden_size; ++hidden) {
                const std::size_t index = static_cast<std::size_t>(
                    time * shape.hidden_size + hidden);
                const float value =
                    quant_lstm::quantization::dequantize(
                        q_output[index],
                        output_params.values.front(),
