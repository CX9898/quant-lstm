...
std::vector<float> canonicalInput(const LstmShape& shape, std::uint64_t seed,
                                  bool batch_first,
    ...
    fillNormalLike(generated.data(), generated.size(), seed,
                   TensorStream::Input);
...
void makeInitialState(const LstmShape& shape, std::uint64_t seed,
    ...
    fillNormalLike(master->h0.data(), count, seed,
                   TensorStream::InitialHidden);
    fillNormalLike(master->c0.data(), count, seed,
...
std::size_t groupCount(QuantOperator id, QuantGranularity granularity,
                       std::size_t channels) {
...
void addParameterRanges(const std::vector<float>& values,
                        std::size_t row_width, std::size_t hidden,
                        QuantOperator id,
...
std::vector<float> dequantizeTensorImpl(
    ...
    for (std::size_t index = 0; index < source.size(); ++index) {
        ...
        result[index] =
            quantization::dequantize(q, point, config.at(id).type);
    }
    return result;
}
...
SyntheticNumericMetrics computeMetrics(const float* actual,
...
LstmShape shapeFromSyntheticProfile(const Json& profile) {
    ...
    return {dimensions.at(0).get<std::int64_t>(),
...
FloatMaster makeFloatMaster(const Json& profile, std::uint64_t data_seed) {
    ...
    if (profile.at("resolved_quant_config").at("scale_mode") == "pot2") {
        const auto snapToPot2Grid = [](std::vector<float>* values) {
            ...
            const float scale =
                std::exp2(std::ceil(std::log2(required_scale)));
            for (float& value : *values) {
...
LstmQuantizationRanges makeSyntheticRanges(
    const Json& profile, const LstmShape& shape,
    const LstmOperatorQuantConfig& config,
...
std::vector<std::int32_t> quantizeTensor(
    ...
    for (std::size_t index = 0; index < source.size(); ++index) {
        ...
        result[index] = quantization::quantize(
...
QuantizedMaster quantizeMaster(const LstmShape& shape,
    ...
    result.weight_ih =
        quantizeTensor(master.weight_ih, QuantOperator::WeightInputHidden,
                       config, params, input_size);
    result.weight_hh =
        quantizeTensor(master.weight_hh, QuantOperator::WeightHiddenHidden,
                       config, params, hidden);
    if (master.bias_enabled) {
        result.bias_ih =
    ...
    if (master.has_explicit_state) {
        ...
        result.c0 = quantizeTensor(master.c0, QuantOperator::CellState, config,
                                   params);
    }
    return result;
}
...
FloatLstmResult runCpuFloatOracle(const LstmShape& shape,
    ...
    lstmForwardFloatCpu(
        ...
        result.output.data(), result.final_hidden.data(),
        result.final_cell.data());
    return result;
}
...
Int32LstmResult runCpuInt32Oracle(
    ...
    lstmForwardInt32CpuReference(
        ...
        master.has_explicit_state ? quantized.h0.data() : nullptr,
        master.has_explicit_state ? quantized.c0.data() : nullptr, config,
        params, execution, result.output.data(), result.final_hidden.data(),
        result.final_cell.data());
    return result;
}
...
FloatLstmResult runCpuFpQuantizedOracle(
    ...
    lstmForwardQuantizedFpCpuReference(
        ...
        {weight_ih.data(), weight_hh.data(),
         master.bias_enabled ? bias_ih.data() : nullptr,
         master.bias_enabled ? bias_hh.data() : nullptr},
        input.data(), master.has_explicit_state ? h0.data() : nullptr,
        master.has_explicit_state ? c0.data() : nullptr, config, params,
        execution, result.output.data(), result.final_hidden.data(),
        result.final_cell.data());
    return result;
}
...
SyntheticPrecisionFixture makeSyntheticPrecisionFixture(const Json& profile) {
    ...
    fixture.config = parseResolvedQuantConfig(
    ...
    fixture.ranges =
        makeSyntheticRanges(profile, fixture.shape, fixture.config,
                            fixture.master);
    fixture.params =
        finalizeQuantParams(fixture.config, fixture.ranges,
                            fixture.shape.hidden_size,
                            fixture.master.bias_enabled);
    fixture.execution =
        deriveLstmExecutionParams(fixture.config, fixture.params,
                                  fixture.shape.input_size);
    fixture.quantized =
        quantizeMaster(fixture.shape, fixture.config, fixture.params,
                       fixture.master);
    fixture.float_oracle = runCpuFloatOracle(fixture.shape, fixture.master);
    fixture.int32_oracle =
        runCpuInt32Oracle(fixture.shape, fixture.config, fixture.params,
                          fixture.execution, fixture.master,
                          fixture.quantized);
    fixture.fp_quantized_oracle =
        runCpuFpQuantizedOracle(fixture.shape, fixture.config, fixture.params,
                                fixture.execution, fixture.master,
                                fixture.quantized);
    return fixture;
}
...
SyntheticNumericMetrics computeSyntheticNumericMetrics(
    ...
    result.saturation_rate =
        saturationRate(quantized_values, count, quantization_type);
    return result;
}
...
TensorMetrics evaluateTensorMetrics(const std::vector<float>& actual,
    ...
    result.metrics =
        computeSyntheticNumericMetrics(actual.data(), expected.data(),
                                       actual.size());
    ...
    result.passed =
        accuracy.mean_absolute_error < thresholds.maximum_mae &&
        accuracy.mean_squared_error < thresholds.maximum_mse &&
        cosine_pass;
    return result;
}
