...
quant_lstm::QuantizedPoint unsignedPoint(float scale,
...
quant_lstm::LstmOperatorQuantConfig makeConfig(
    ...
    for (std::size_t index = 0; index < quant_lstm::kQuantOperatorCount; ++index) {
        ...
        config.operators[index].granularity =
...
quant_lstm::LstmQuantParams makeQuantParams(
    ...
    for (std::size_t index = 0; index < quant_lstm::kQuantOperatorCount; ++index) {
        ...
        const std::size_t count =
            quant_lstm::isParameterOperator(id) ? 4 : 1;
...
int main() {
    try {
        ...
        const auto integer =
            quant_lstm::computeQuantizedCellInt(4, 5, 8, -1, cell_params);
        require(integer.value == 9, "Cell int 双比例融合");
        require(integer.diagnostics.p_forget == 32 &&
                    integer.diagnostics.p_input == -8 &&
                    integer.diagnostics.pre_round_sum ==
                        static_cast<__int128>(24) *
                            (std::int64_t{1} << 30),
                "Cell int diagnostics");

        const auto fp = quant_lstm::computeQuantizedCellFp(
            4.0F, 5.0F, 8.0F, -1.0F, cell_params);
        require(fp.value == static_cast<float>(integer.value) &&
                    fp.diagnostics.pre_round_sum == 12.0F,
                "Cell FP 模拟相同 Q31 编码");
        ...
        const auto tie = quant_lstm::computeQuantizedCellInt(
            1, 1, 1, 1, delayed_round);
        require(tie.value == 1,
                "Cell 必须合并后只做一次 RoundShift31");
        ...
        require(quant_lstm::computeQuantizedCellInt(
                    127, 127, 127, 127, saturated)
                    .value == 127,
                "Cell 最终边界饱和");
        ...
        hidden.product_to_output.kind =
            quant_lstm::ExecutionRescaleKind::MShift;
        hidden.product_to_output.m_shift = {32768, 16};
        const auto hidden_int =
            quant_lstm::computeQuantizedHiddenInt(3, 5, hidden);
        const auto hidden_fp =
            quant_lstm::computeQuantizedHiddenFp(3.0F, 5.0F, hidden);
        ...
        requireThrows(
            [&] {
                static_cast<void>(
                    quant_lstm::computeQuantizedCellInt(-1, 0, 0, 0,
                                                        cell_params));
            },
            "unsigned 门输入越界必须失败");
        requireThrows(
            [&] {
                static_cast<void>(
                    quant_lstm::computeQuantizedCellFp(
                        1.5F, 0.0F, 0.0F, 0.0F, cell_params));
            },
        ...
        for (const auto mode :
            ...
            const auto execution =
                quant_lstm::deriveLstmExecutionParams(config, params, 2, true);
            const auto expected_kind =
                mode == quant_lstm::quantization::ScaleMode::Affine
                    ? quant_lstm::ExecutionRescaleKind::MShift
                    : quant_lstm::ExecutionRescaleKind::Pot2;
            require(execution.input_hidden_linear.weights.size() == 4 &&
                        execution.gates.size() == 4 &&
                        execution.input_hidden_linear
                                .accumulator_to_output.front()
                                .kind == expected_kind &&
                        execution.hidden.product_to_output.kind ==
                            expected_kind,
                    "集中派生 Linear/四门/Hidden 编码");
            require(execution.cell.forget_scale.multiplier ==
                        (std::int64_t{1} << 24) &&
                        execution.cell.input_scale.multiplier ==
                            (std::int64_t{1} << 24) &&
                        execution.diagnostics.cell.safe() &&
                        execution.diagnostics.hidden.safe(),
                    "集中派生 Q31 与 NumericSafety diagnostics");
        }

        const auto config =
            makeConfig(quant_lstm::quantization::ScaleMode::Affine);
        const auto params = makeQuantParams(config);
        requireThrows(
            [&] {
                static_cast<void>(
                    quant_lstm::deriveLstmExecutionParams(config, params, 0));
            },
            "非法 input_size 必须失败");
        requireThrows(
            [&] {
                static_cast<void>(quant_lstm::deriveLstmExecutionParams(
                    config, params, 2000, true));
            },
