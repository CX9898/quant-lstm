...
int main() {
    try {
        ...
        const float first_values[]{
            ...
            0.6F, 0.7F, 0.8F, 100.0F,
            std::numeric_limits<float>::quiet_NaN(),
            std::numeric_limits<float>::infinity(),
        };
        first.collect(first_values,
                      sizeof(first_values) / sizeof(first_values[0]));
        require(first.histogram().total_count == 10,
                "histogram must ignore NaN and infinity");
        ...
        require(first.histogram().total_count == 12 &&
                    first.histogram().minimum <= -2.0F &&
                    first.histogram().maximum >= 100.0F,
                "histogram merge must preserve range and count");

        const auto percentile = calibrateHistogramRange(
            first.histogram(), {8, false, false},
            HistogramCalibrationMethod::Percentile,
            HistogramCalibrationOptions{80.0F, 101, 17, 21});
        require(percentile.first < percentile.second &&
                    percentile.second < first.histogram().maximum,
                "percentile must independently clip sparse outlier bins");

        const auto sqnr = calibrateHistogramRange(
            first.histogram(), {8, false, true},
            HistogramCalibrationMethod::Sqnr);
        require(std::isfinite(sqnr.first) && std::isfinite(sqnr.second) &&
                    sqnr.first < sqnr.second &&
                    std::abs(sqnr.first) == std::abs(sqnr.second),
        ...
        for (CalibrationMethod method :
             {CalibrationMethod::Sqnr, CalibrationMethod::Percentile}) {
            LstmCalibrationSession session(defaultConfig(), 1, 1, false,
                                           method, {}, 64);
            ...
            require(session.state() == CalibrationState::Locked &&
                        finalized.report.method == method,
            ...
            for (std::size_t index = 0; index < kQuantOperatorCount; ++index) {
                ...
                const auto& collectors =
                    session.collector().histograms()[index];
                require(!collectors.empty() &&
                            !collectors.front().histogram().empty(),
                        "all enabled histogram quant points must collect");
                for (const auto& value :
                     finalized.quant_params.operators[index].values) {
