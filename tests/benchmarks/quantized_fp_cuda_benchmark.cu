...
int parseInteger(const char* text, const char* name, bool allow_zero) {
    ...
    if (parsed.ec != std::errc{} ||
        ...
        throw std::invalid_argument(std::string(name) + " 必须为" +
                                    (allow_zero ? "非负" : "正") +
                                    "整数");
    }
    return value;
...
Arguments parseArguments(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (option == "--device") {
            result.device =
                parseInteger(argv[++index], "device", true);
        } else if (option == "--warmup-iterations") {
            result.warmup =
                parseInteger(argv[++index], "warmup", true);
        } else if (option == "--iterations") {
            result.iterations =
                parseInteger(argv[++index], "iterations", false);
...
Json readJson(const std::filesystem::path& path) {
    ...
    if (!stream) {
        throw std::runtime_error("无法读取 benchmark fixture: " +
                                 path.string());
    }
    Json result;
    stream >> result;
...
std::vector<Json> benchmarkProfiles(const Json& matrix) {
    ...
    constexpr std::array<const char*, 2> kShapes{
        "cuda_gru_basic", "large_batch"};
    constexpr std::array<const char*, 2> kModes{"pedantic", "tf32"};
    for (const char* shape : kShapes) {
        for (const char* mode : kModes) {
            const auto match = std::find_if(
                matrix.at("cases").begin(), matrix.at("cases").end(),
                [&](const Json& item) {
                    return item.at("backend") == "cuda_fp32" &&
                           item.at("shape_profile") == shape &&
                           item.at("math_mode") == mode;
                });
            if (match == matrix.at("cases").end()) {
                throw std::runtime_error(
                    std::string("strict_matrix_v2 缺少 benchmark profile: ") +
                    shape + "/" + mode);
            }
    ...
    EventSet() {
        try {
            for (cudaEvent_t& event : values) {
                quant_lstm::test::checkCuda(cudaEventCreate(&event),
                                            "cudaEventCreate timing");
            }
        } catch (...) {
            release();
            throw;
...
Json metricsJson(
    const quant_lstm::test::SyntheticNumericMetrics& metrics) {
    ...
    result["cosine_similarity"] =
        accuracy.cosine_not_applicable || accuracy.one_sided_zero_norm
            ? Json(nullptr)
...
Json runCase(const Json& profile, const Arguments& arguments) {
    ...
    const auto forward = [&](const auto* timing) {
        ...
        quant_lstm::lstmForwardQuantizedFpCuda(
            ...
            input.get(),
            fixture.master.has_explicit_state ? h0.get() : nullptr,
            fixture.master.has_explicit_state ? c0.get() : nullptr,
    ...
    for (int index = 0; index < arguments.warmup; ++index) {
        forward(
            static_cast<const quant_lstm::
                            LstmQuantizedFpCudaTimingEvents*>(nullptr));
    }
    context.synchronize();
    ...
    for (int index = 0; index < arguments.iterations; ++index) {
        ...
        forward(&timing);
        quant_lstm::test::checkCuda(
            cudaEventSynchronize(timing.complete),
            "cudaEventSynchronize benchmark");
        end_to_end.push_back(elapsed(timing.start, timing.complete));
        quantization.push_back(
            elapsed(timing.start, timing.quantization_complete));
        core.push_back(elapsed(timing.quantization_complete,
                               timing.core_complete));
    }
    ...
    const auto output_metrics =
        quant_lstm::test::computeSyntheticNumericMetrics(
            actual_output.data(), fixture.float_oracle.output.data(),
            output_count, quantized_output.data(),
            fixture.config.at(quant_lstm::QuantOperator::Output).type);
    const auto hidden_metrics =
        quant_lstm::test::computeSyntheticNumericMetrics(
            actual_hn.data(), fixture.float_oracle.final_hidden.data(),
            state_count, quantized_hn.data(),
            fixture.config.at(quant_lstm::QuantOperator::Output).type);
    const auto cell_metrics =
        quant_lstm::test::computeSyntheticNumericMetrics(
            actual_cn.data(), fixture.float_oracle.final_cell.data(),
            state_count, quantized_cn.data(),
    ...
    return {
        ...
        {"fp32_accumulation_class",
         profile.at("fp32_accumulation_class")},
...
Json environmentJson(int device) {
    ...
    int runtime = 0;
    int cublas = 0;
    quant_lstm::test::checkCuda(cudaDriverGetVersion(&driver),
                                "cudaDriverGetVersion");
    quant_lstm::test::checkCuda(cudaRuntimeGetVersion(&runtime),
                                "cudaRuntimeGetVersion");
    cublasHandle_t handle = nullptr;
    quant_lstm::test::checkCublas(cublasCreate(&handle), "cublasCreate");
    try {
        quant_lstm::test::checkCublas(
            cublasGetVersion(handle, &cublas), "cublasGetVersion");
    } catch (...) {
...
int main(int argc, char** argv) {
    try {
        ...
        quant_lstm::test::checkCuda(cudaSetDevice(arguments.device),
                                    "cudaSetDevice");
        ...
        for (const Json& profile : benchmarkProfiles(matrix)) {
            ...
            total_profiled_gemm_calls +=
                ...
                static_cast<std::uint64_t>(arguments.iterations +
                                           arguments.warmup);
        }
        const Json report{
            {"schema_version", 2},
