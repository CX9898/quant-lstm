...
Arguments parseArguments(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if ((option == "--selection" || option == "--report") &&
            index + 1 >= argc) {
...
std::vector<float> sliceLastState(const std::vector<float>& source,
                                  std::size_t state_count) {
    if (source.size() < state_count) {
        throw std::invalid_argument("checkpoint state 长度非法");
    }
    return {source.end() - static_cast<std::ptrdiff_t>(state_count),
            source.end()};
}

void requireExact(const std::vector<float>& actual,
...
bool executionIsExact(const quant_lstm::LstmExecutionParams& execution) {
    ...
    return std::all_of(
               execution.diagnostics.input_hidden_linear.begin(),
               execution.diagnostics.input_hidden_linear.end(), exact) &&
           std::all_of(
               execution.diagnostics.hidden_hidden_linear.begin(),
               execution.diagnostics.hidden_hidden_linear.end(), exact) &&
           std::all_of(execution.diagnostics.gates.begin(),
                       execution.diagnostics.gates.end(), exact) &&
           exact(execution.diagnostics.cell) &&
           exact(execution.diagnostics.hidden);
}

bool tf32OperandsAreExact(
...
Json metricJson(const quant_lstm::test::SyntheticNumericMetrics& metrics) {
    Json result{
        {"max_absolute_error",
         metrics.accuracy.maximum_absolute_error},
...
bool passes(const quant_lstm::test::SyntheticNumericMetrics& metrics,
            const Json& threshold) {
    const auto& accuracy = metrics.accuracy;
    const bool cosine =
        accuracy.cosine_not_applicable
            ? accuracy.mean_absolute_error == 0.0 &&
                  accuracy.mean_squared_error == 0.0
...
std::vector<Json> selectProfiles(const Json& matrix,
                                 const std::string& selection) {
    std::vector<Json> candidates;
    for (const Json& item : matrix.at("cases")) {
        if (item.at("backend") == "cuda_fp32" &&
            item.at("tier") == selection) {
    ...
    if (selection == "basic") {
        ...
        const auto pedantic = std::find_if(
            candidates.begin(), candidates.end(), [](const Json& item) {
                return item.at("math_mode") == "pedantic";
            });
        return {pedantic == candidates.end() ? candidates.front()
                                             : *pedantic};
    }
    ...
    for (const char* case_id : kCaseIds) {
        ...
        if (match == candidates.end()) {
            throw std::runtime_error(
                std::string("v2 matrix 缺少定向 CUDA strict case: ") +
                case_id);
        }
...
Json runProfile(const Json& profile, const Json& thresholds,
    ...
    quant_lstm::LstmQuantizedFpCudaCheckpoints checkpoints{
        linear_ih.get(), linear_hh.get(), gate_inputs.get(),
        gate_outputs.get(), cell_states.get(), cell_tanh.get(),
        hidden_outputs.get()};
    ...
    DeviceBuffer<std::byte> workspace(
        caller_workspace ? required_workspace : 0);
    ...
    quant_lstm::lstmForwardQuantizedFpCuda(
        ...
        input.get(),
        fixture.master.has_explicit_state ? initial_hidden.get() : nullptr,
        fixture.master.has_explicit_state ? initial_cell.get() : nullptr,
        fixture.config, fixture.params, fixture.execution, output.get(),
        final_hidden.get(), final_cell.get(), context.get(),
    ...
    quant_lstm::test::FloatLstmResult actual{
        output.copyToHost(), final_hidden.copyToHost(),
    ...
    quant_lstm::lstmForwardQuantizedFpCpuReference(
        ...
        q_input.data(),
        fixture.master.has_explicit_state ? q_h0.data() : nullptr,
        fixture.master.has_explicit_state ? q_c0.data() : nullptr,
        fixture.config, fixture.params, fixture.execution,
        cpu_q_output.data(), cpu_q_hidden.data(), cpu_q_cell.data(),
        &cpu_trace);

    const bool derived_exact =
        executionIsExact(fixture.execution) &&
        (pedantic || tf32OperandsAreExact(fixture.config));
    const std::string derived_class =
        derived_exact ? "exact_integer_range" : "precision_risk";
    if (profile.at("fp32_accumulation_class") != derived_class) {
        throw std::runtime_error(
            profile.at("case_id").get<std::string>() +
            " matrix NumericSafety 标签与派生报告不一致");
    }
    if (derived_exact) {
        requireExact(q_linear_ih, cpu_trace.weight_ih_linear,
                     "weight_ih_linear");
        requireExact(q_linear_hh, cpu_trace.weight_hh_linear,
                     "weight_hh_linear");
        requireExact(q_gate_inputs, cpu_trace.gate_inputs, "gate_inputs");
        requireExact(q_gate_outputs, cpu_trace.gate_outputs,
                     "gate_outputs");
        requireExact(q_cell, cpu_trace.cell_states, "cell");
        requireExact(q_cell_tanh, cpu_trace.cell_tanh_outputs,
                     "cell_tanh");
        requireExact(q_hidden, cpu_trace.hidden_outputs, "output");
        requireExact(sliceLastState(q_hidden, state_count), cpu_q_hidden,
                     "h_n");
        requireExact(sliceLastState(q_cell, state_count), cpu_q_cell,
                     "c_n");
    }

    const auto output_metrics =
        quant_lstm::test::computeSyntheticNumericMetrics(
            actual.output.data(), fixture.float_oracle.output.data(),
            output_count, q_hidden.data(),
    ...
    const auto hidden_metrics =
        quant_lstm::test::computeSyntheticNumericMetrics(
            ...
            fixture.float_oracle.final_hidden.data(), state_count,
            hidden_q.data(),
            fixture.config.at(quant_lstm::QuantOperator::Output).type);
    const auto cell_metrics =
        quant_lstm::test::computeSyntheticNumericMetrics(
            actual.final_cell.data(),
            fixture.float_oracle.final_cell.data(), state_count,
            cell_q.data(),
            fixture.config.at(quant_lstm::QuantOperator::CellState).type);
    const Json& threshold =
        thresholds.at("profiles").at(profile.at("threshold_profile"));
    const bool passed = passes(output_metrics, threshold) &&
                        passes(hidden_metrics, threshold) &&
                        passes(cell_metrics, threshold);
    if (!passed) {
        throw std::runtime_error(
            profile.at("case_id").get<std::string>() +
            " 未通过既有 strict threshold: output=" +
            metricJson(output_metrics).dump() + ", h_n=" +
            metricJson(hidden_metrics).dump() + ", c_n=" +
            metricJson(cell_metrics).dump());
    }
    ...
    return {
        ...
        {"workspace",
         {{"kind", caller_workspace ? "caller" : "internal"},
          ...
          {"breakdown",
           {{"quantized_input_bytes",
             workspace_breakdown.quantized_input_bytes},
            {"device_parameter_bytes",
             workspace_breakdown.device_parameter_bytes}}}}},
        {"gemm_calls",
         {{"input", stats.input_gemm_calls},
          {"recurrent", stats.recurrent_gemm_calls}}},
        {"metrics",
         {{"output", metricJson(output_metrics)},
          {"h_n", metricJson(hidden_metrics)},
          {"c_n", metricJson(cell_metrics)}}},
        {"numeric_safety",
         quant_lstm::test::executionSafetyJson(fixture.execution)},
        {"passed", true},
    };
}
