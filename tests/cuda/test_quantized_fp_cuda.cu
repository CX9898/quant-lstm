...
std::vector<float> floatValues(const Json& tensors,
...
GoldenCase loadCase(const Json& tensors) {
    ...
    result.config.scale_mode = quant_lstm::parseScaleMode(
        data(tensors, "scale_mode").at(0).get<std::string>());
    const auto names = values<std::string>(tensors, "operator_names");
    const auto bitwidth = values<std::uint8_t>(
        tensors, "operator_bitwidth");
    const auto is_unsigned = values<bool>(
        tensors, "operator_is_unsigned");
    const auto is_symmetric = values<bool>(
        tensors, "operator_is_symmetric");
    const auto granularities = values<std::string>(
        tensors, "operator_granularity");
    const auto offsets = values<std::int32_t>(
        tensors, "quant_param_value_offsets");
    const auto group_counts = values<std::int32_t>(
        tensors, "quant_param_group_counts");
    const auto scales = floatValues(tensors, "quant_param_scales");
    const auto zero_points = values<std::int32_t>(
        tensors, "quant_param_zero_points");
    ...
    result.params.bias_enabled =
        data(tensors, "bias_enabled").at(0).get<bool>();
    for (std::size_t index = 0;
         index < quant_lstm::kQuantOperatorCount; ++index) {
        const auto id = static_cast<quant_lstm::QuantOperator>(index);
        require(names[index] == quant_lstm::quantOperatorName(id),
                "Golden operator 顺序非法");
        auto& operator_config = result.config.at(id);
        operator_config.type = {
            bitwidth.at(index), is_unsigned.at(index),
            is_symmetric.at(index)};
        operator_config.granularity =
            quant_lstm::parseGranularity(granularities.at(index));
        auto& operator_params = result.params.operators[index];
        operator_params.source_granularity =
            operator_config.granularity;
        for (std::int32_t offset = offsets.at(index);
             offset < offsets.at(index + 1); ++offset) {
            operator_params.values.push_back(
                {scales.at(static_cast<std::size_t>(offset)),
                 zero_points.at(static_cast<std::size_t>(offset))});
        }
        operator_params.group_diagnostics.resize(
            static_cast<std::size_t>(group_counts.at(index)));
    }
    result.config.validate();
    result.params.validate(result.config);
    result.execution = quant_lstm::deriveLstmExecutionParams(
        result.config, result.params, result.shape.input_size);
...
std::vector<float> dequantize(
    ...
    quant_lstm::QuantOperator id, const GoldenCase& fixture,
    ...
    for (std::size_t index = 0; index < source.size(); ++index) {
        ...
        result[index] = quant_lstm::quantization::dequantize(
            source[index], points.at(parameter_index),
            fixture.config.at(id).type);
    }
    return result;
}

template <typename Expected>
void compareExact(const std::vector<float>& actual,
...
std::vector<std::int32_t> flattenGateCheckpoints(
    const Json& checkpoints, std::string_view suffix,
...
void runGolden(const Json& document, bool caller_workspace,
               bool test_small_workspace) {
    ...
    const std::size_t steps =
        static_cast<std::size_t>(fixture.shape.sequence_length);
    const std::size_t batch =
        static_cast<std::size_t>(fixture.shape.batch_size);
    const std::size_t input_size =
        static_cast<std::size_t>(fixture.shape.input_size);
    const std::size_t hidden =
    ...
    const auto input = dequantize(
        fixture.input, quant_lstm::QuantOperator::Input, fixture);
    const auto weight_ih = dequantize(
        fixture.weight_ih,
        quant_lstm::QuantOperator::WeightInputHidden, fixture,
        input_size);
    const auto weight_hh = dequantize(
        fixture.weight_hh,
        quant_lstm::QuantOperator::WeightHiddenHidden, fixture, hidden);
    const auto bias_ih = dequantize(
        fixture.bias_ih, quant_lstm::QuantOperator::BiasInputHidden,
        fixture, 1);
    const auto bias_hh = dequantize(
        fixture.bias_hh, quant_lstm::QuantOperator::BiasHiddenHidden,
        fixture, 1);
    const auto h0 = dequantize(
        fixture.h0, quant_lstm::QuantOperator::Output, fixture);
    const auto c0 = dequantize(
        fixture.c0, quant_lstm::QuantOperator::CellState, fixture);
    ...
    quant_lstm::LstmQuantizedFpCudaCheckpoints checkpoints{
        d_linear_ih.get(), d_linear_hh.get(), d_gate_input.get(),
        d_gate_output.get(), d_cell.get(), d_cell_tanh.get(),
        d_hidden.get()};
    const std::size_t workspace_bytes =
        quant_lstm::lstmQuantizedFpCudaWorkspaceBytes(
    ...
    if (test_small_workspace) {
        ...
        try {
            quant_lstm::lstmForwardQuantizedFpCuda(
                ...
                fixture.params, fixture.execution, d_output.get(),
                d_hn.get(), d_cn.get(), context.get(),
    ...
    compareExact(d_linear_hh.copyToHost(),
                 values<std::int32_t>(expected, "weight_hh_linear"),
                 case_id + " weight_hh_linear");
    compareExact(d_gate_input.copyToHost(),
                 flattenGateCheckpoints(expected, "gate_input",
                                        fixture.shape),
                 case_id + " gate_input");
    compareExact(d_gate_output.copyToHost(),
                 flattenGateCheckpoints(expected, "gate_output",
                                        fixture.shape),
                 case_id + " gate_output");
    compareExact(d_cell.copyToHost(),
                 values<std::int32_t>(expected, "cell"),
                 case_id + " cell");
    compareExact(d_cell_tanh.copyToHost(),
                 values<std::int32_t>(expected, "cell_tanh"),
                 case_id + " cell_tanh");
    compareExact(d_hidden.copyToHost(),
                 values<std::int32_t>(expected, "output"),
                 case_id + " output q");
...
int main() {
    try {
        ...
        for (const auto& fixture :
             quant_lstm::test::kGoldenDocuments) {
            if (fixture.execution_model != "cpu_fp32" ||
                (fixture.kind != "cell" &&
                 fixture.kind != "recurrent")) {
                continue;
            }
            runGolden(Json::parse(fixture.json), checked % 2 == 0,
                      checked == 0);
            ++checked;
        }
        require(checked == 2,
                "CUDA Golden 必须复用两个 cpu_fp32 reference 用例");
