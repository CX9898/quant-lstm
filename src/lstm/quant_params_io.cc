...
std::size_t groupCountForDiagnostics(
    QuantOperator id, QuantGranularity granularity, std::int64_t hidden_size) {
...
OperatorQuantConfig parseOperatorConfig(QuantOperator id, const Json& value) {
    rejectUnknownKeys(value,
                      {"bitwidth", "is_unsigned", "is_symmetric",
                       "granularity", "scales", "zero_points"},
    ...
    result.granularity =
        parseGranularity(requiredString(value, "granularity"));
    return result;
}

FinalizedOperatorQuantParams parseOperatorParams(
    QuantOperator id, const Json& value, const OperatorQuantConfig& config,
    ...
    for (std::size_t index = 0; index < expected; ++index) {
        ...
        result.values.push_back(
            {parseCanonicalFloat(scales[index]),
             static_cast<std::int32_t>(zero_point)});
    }
    result.group_diagnostics.resize(
        groupCountForDiagnostics(id, config.granularity, hidden_size));
    return result;
}
...
void LstmQuantParamsBundle::validate() const {
    if (schema_version != 1) {
        throw std::invalid_argument(
            "只支持 quant params bundle schema_version=1");
    }
    if (input_size <= 0) {
...
std::string exportQuantParamsBundle(const LstmQuantParamsBundle& bundle) {
    ...
    for (std::size_t index = 0; index < kQuantOperatorCount; ++index) {
        ...
        operators[std::string(quantOperatorName(id))] =
            operatorJson(bundle.config.operators[index],
                         bundle.quant_params.operators[index]);
    }
...
LstmQuantParamsBundle importQuantParamsBundle(std::string_view json_text,
    ...
    try {
        ...
        throw std::invalid_argument(std::string("quant params JSON 解析失败: ") +
                                    error.what());
    }
    rejectUnknownKeys(root,
                      {"schema_version", "input_size", "hidden_size",
                       "bias_enabled", "scale_mode", "operators"},
                      "quant params root");
    ...
    result.quant_params.hidden_size =
        requiredInteger<std::int64_t>(root, "hidden_size");
    ...
    result.quant_params.bias_enabled =
        requiredBoolean(root, "bias_enabled");
    result.config.schema_version = result.schema_version;
    result.config.scale_mode =
        parseScaleMode(requiredString(root, "scale_mode"));
    ...
    for (std::size_t index = 0; index < kQuantOperatorCount; ++index) {
        ...
        result.quant_params.operators[index] = parseOperatorParams(
            id, value, result.config.operators[index],
            result.quant_params.hidden_size);
    }
    for (const auto& [name, unused] : root["operators"].items()) {
        static_cast<void>(unused);
        if (expected_names.count(name) == 0U) {
            throw std::invalid_argument("operators 包含非法或禁用字段: " +
                                        name);
        }
    }
    result.validate();
...
LstmExecutionParams auditQuantParamsBundle(
    const LstmQuantParamsBundle& bundle, bool require_exact_accumulation) {
    bundle.validate();
    return deriveLstmExecutionParams(
        bundle.config, bundle.quant_params, bundle.input_size,
