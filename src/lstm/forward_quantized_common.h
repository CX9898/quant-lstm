...
inline std::size_t operatorIndex(QuantOperator id) {
    return static_cast<std::size_t>(id);
}

inline const QuantizedPoint& pointAt(const LinearExecutionParams& linear,
                                     bool weight, std::size_t channel) {
...
inline float clampFp(float value, const QuantizedPoint& point) {
    ...
    const auto range = point.type.range();
    return std::clamp(value, static_cast<float>(range.minimum),
...
inline void requireGrid(float value, const QuantizedPoint& point) {
    ...
    if (!std::isfinite(value) ||
        ...
        value < static_cast<float>(range.minimum) ||
        value > static_cast<float>(range.maximum)) {
...
inline void validateCommon(
    const LstmShape& shape, bool bias_enabled, const void* weight_ih,
    ...
    if (weight_ih == nullptr || weight_hh == nullptr || input == nullptr ||
