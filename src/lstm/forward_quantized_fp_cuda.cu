...
std::size_t checkedMul(std::size_t lhs, std::size_t rhs,
                       const char* description) {
...
WorkspaceLayout makeWorkspaceLayout(const LstmShape& shape,
                                    bool bias_enabled) {
    const std::size_t steps = positiveSize(shape.sequence_length,
                                           "sequence_length");
    ...
    result.quantized_input_bytes =
        floatBytes(checkedMul(sequence_batch, input, "T*B*I"),
                   "quantized input");
    result.quantized_weight_ih_bytes =
        floatBytes(checkedMul(channels, input, "4H*I"),
                   "quantized weight_ih");
    result.quantized_weight_hh_bytes =
        floatBytes(checkedMul(channels, hidden, "4H*H"),
                   "quantized weight_hh");
    result.quantized_bias_bytes =
        bias_enabled ? floatBytes(checkedMul(2, channels, "2*4H"),
                                  "quantized bias")
                     : 0;
    result.quantized_state_bytes =
        floatBytes(checkedMul(2, states, "2*B*H"), "quantized states");
    result.input_linear_bytes =
        floatBytes(checkedMul(sequence_batch, channels, "T*B*4H"),
                   "input linear");
    result.recurrent_linear_bytes =
        floatBytes(checkedMul(batch, channels, "B*4H"),
                   "recurrent linear");
    result.weight_sum_bytes =
        floatBytes(checkedMul(2, channels, "2*4H"), "weight sums");
    result.device_parameter_bytes =
        checkedMul(channels, sizeof(DeviceLinearChannelParams),
                   "device linear parameters");

    std::size_t cursor = 0;
    appendRegion(result.quantized_input_bytes, &cursor,
                 &layout.quantized_input);
    appendRegion(result.quantized_weight_ih_bytes, &cursor,
                 &layout.quantized_weight_ih);
    appendRegion(result.quantized_weight_hh_bytes, &cursor,
                 &layout.quantized_weight_hh);
    appendRegion(result.quantized_bias_bytes, &cursor,
                 &layout.quantized_bias);
    appendRegion(result.quantized_state_bytes, &cursor,
                 &layout.quantized_state);
    appendRegion(result.input_linear_bytes, &cursor,
                 &layout.input_linear);
    appendRegion(result.recurrent_linear_bytes, &cursor,
                 &layout.recurrent_linear);
    appendRegion(result.weight_sum_bytes, &cursor, &layout.weight_sum);
    appendRegion(result.device_parameter_bytes, &cursor,
                 &layout.device_parameters);
    result.total_bytes = alignUp(cursor);

    std::size_t payload = 0;
    payload = checkedAdd(payload, result.quantized_input_bytes, "workspace");
    payload =
        checkedAdd(payload, result.quantized_weight_ih_bytes, "workspace");
    payload =
        checkedAdd(payload, result.quantized_weight_hh_bytes, "workspace");
...
bool samePoint(const QuantizedPoint& lhs, const QuantizedPoint& rhs) {
    return lhs.param.scale == rhs.param.scale &&
           lhs.param.zero_point == rhs.param.zero_point &&
           sameType(lhs.type, rhs.type);
}

bool sameRescale(const ExecutionRescale& lhs,
                 const ExecutionRescale& rhs) {
...
bool sameLinear(const LinearExecutionParams& lhs,
                const LinearExecutionParams& rhs) {
    if (!samePoint(lhs.input, rhs.input) ||
        !samePoint(lhs.output, rhs.output) ||
        lhs.weights.size() != rhs.weights.size() ||
        lhs.biases.size() != rhs.biases.size() ||
        lhs.bias_to_accumulator.size() !=
            rhs.bias_to_accumulator.size() ||
        lhs.accumulator_to_output.size() !=
            rhs.accumulator_to_output.size()) {
        return false;
    }
    for (std::size_t index = 0; index < lhs.weights.size(); ++index) {
        if (!samePoint(lhs.weights[index], rhs.weights[index]) ||
            !sameRescale(lhs.accumulator_to_output[index],
                         rhs.accumulator_to_output[index])) {
            return false;
        }
    }
    for (std::size_t index = 0; index < lhs.biases.size(); ++index) {
        if (!samePoint(lhs.biases[index], rhs.biases[index]) ||
            !sameRescale(lhs.bias_to_accumulator[index],
                         rhs.bias_to_accumulator[index])) {
            return false;
        }
    }
    return true;
...
void validateExecutionParams(
    ...
    if (shape.input_size != actual.input_size ||
        shape.hidden_size != actual.hidden_size ||
        shape.hidden_size != quant_params.hidden_size) {
        throw std::invalid_argument(
            "shape、LstmQuantParams 与 LstmExecutionParams 不一致");
    }

    const LstmExecutionParams expected =
        deriveLstmExecutionParams(config, quant_params, shape.input_size);
    if (!sameLinear(actual.input_hidden_linear,
                    expected.input_hidden_linear) ||
        !sameLinear(actual.hidden_hidden_linear,
                    expected.hidden_hidden_linear)) {
    ...
    for (std::size_t gate = 0; gate < kGateCount; ++gate) {
        ...
        if (!samePoint(lhs.input_hidden_linear,
                       rhs.input_hidden_linear) ||
            !samePoint(lhs.hidden_hidden_linear,
                       rhs.hidden_hidden_linear) ||
            !samePoint(lhs.gate_input, rhs.gate_input) ||
            !samePoint(lhs.gate_output, rhs.gate_output) ||
            !sameRescale(lhs.input_hidden_to_gate,
                         rhs.input_hidden_to_gate) ||
            !sameRescale(lhs.hidden_hidden_to_gate,
                         rhs.hidden_hidden_to_gate)) {
            throw std::invalid_argument(
                "Gate 执行参数不是 resolved 参数的派生结果");
        }
    }
    ...
    if (!samePoint(lhs_cell.forget_gate, rhs_cell.forget_gate) ||
        ...
        lhs_cell.forget_scale.multiplier !=
            rhs_cell.forget_scale.multiplier ||
        lhs_cell.input_scale.multiplier !=
            rhs_cell.input_scale.multiplier) {
        throw std::invalid_argument(
            "Cell Q31 执行参数不是 resolved 参数的派生结果");
    }
    if (!samePoint(actual.hidden.output_gate,
                   expected.hidden.output_gate) ||
        !samePoint(actual.hidden.cell_tanh,
                   expected.hidden.cell_tanh) ||
        !samePoint(actual.hidden.output, expected.hidden.output) ||
        !sameRescale(actual.hidden.product_to_output,
                     expected.hidden.product_to_output)) {
        throw std::invalid_argument(
            "Hidden 执行参数不是 resolved 参数的派生结果");
    }
}
...
DeviceRescale packRescale(const ExecutionRescale& value) {
    if (value.kind == ExecutionRescaleKind::MShift) {
        if (value.m_shift.multiplier < 32768U) {
            throw std::invalid_argument(
                "M+shift multiplier 未规范化");
        }
        return {DeviceRescaleKind::MShift, value.m_shift.multiplier,
                value.m_shift.shift};
    }
...
std::vector<DeviceLinearChannelParams> packLinearParameters(
    const LstmExecutionParams& params, bool bias_enabled) {
    const std::size_t channels =
        params.input_hidden_linear.weights.size();
    std::vector<DeviceLinearChannelParams> packed(channels);
    for (std::size_t channel = 0; channel < channels; ++channel) {
        auto& destination = packed[channel];
        destination.weight_ih =
            packPoint(params.input_hidden_linear.weights[channel]);
        destination.weight_hh =
            packPoint(params.hidden_hidden_linear.weights[channel]);
        destination.input_linear =
            packPoint(params.input_hidden_linear.output);
        destination.recurrent_linear =
            packPoint(params.hidden_hidden_linear.output);
        destination.input_accumulator_to_linear = packRescale(
            params.input_hidden_linear.accumulator_to_output[channel]);
        destination.recurrent_accumulator_to_linear = packRescale(
            params.hidden_hidden_linear.accumulator_to_output[channel]);
        if (bias_enabled) {
            destination.bias_ih =
                packPoint(params.input_hidden_linear.biases[channel]);
            destination.bias_hh =
                packPoint(params.hidden_hidden_linear.biases[channel]);
            destination.bias_ih_to_accumulator = packRescale(
                params.input_hidden_linear
                    .bias_to_accumulator[channel]);
            destination.bias_hh_to_accumulator = packRescale(
                params.hidden_hidden_linear
                    .bias_to_accumulator[channel]);
        } else {
...
DeviceScalarParams packScalarParameters(
    const LstmExecutionParams& params, bool bias_enabled) {
    ...
    for (std::size_t gate = 0; gate < kGateCount; ++gate) {
        result.gates[gate] = {
            packPoint(params.gates[gate].gate_input),
    ...
    result.hidden_product_to_output =
        packRescale(params.hidden.product_to_output);
    if (params.cell.forget_scale.multiplier < 0 ||
        params.cell.input_scale.multiplier < 0) {
...
void validateContext(const LstmQuantizedFpCudaContext& context) {
    if (context.handles[0] == context.handles[1] ||
        context.streams[0] == context.streams[1] ||
        context.events[0] == context.events[1]) {
        throw std::invalid_argument(
            "CUDA context 必须提供两个不同的 handle/stream/event");
    }
    ...
    for (std::size_t index = 0; index < events.size(); ++index) {
        ...
        for (std::size_t previous = 0; previous < index; ++previous) {
            if (events[index] == events[previous]) {
                throw std::invalid_argument(
                    "timing CUDA events 必须互不相同");
            }
        }
    }
}

void validateDeviceSpan(const void* pointer, std::size_t bytes,
                        int expected_device, const char* name) {
    if (pointer == nullptr) {
        throw std::invalid_argument(std::string(name) + " 不能为空");
    }
...
void validateOptionalDeviceSpan(const void* pointer, std::size_t bytes,
                                int expected_device, const char* name) {
    if (pointer != nullptr) {
...
unsigned int oneDimensionalBlocks(std::size_t elements) {
    const std::size_t needed =
        (elements + static_cast<std::size_t>(kThreads) - 1) /
        static_cast<std::size_t>(kThreads);
    return static_cast<unsigned int>(
        std::min<std::size_t>(needed, kMaximumOneDimensionalBlocks));
}
...
__device__ __forceinline__ float quantizeMasterDevice(
    float value, const DeviceQuantPoint& point) {
    const double translated =
        static_cast<double>(value) / static_cast<double>(point.scale) +
...
__device__ __forceinline__ float dequantizeDevice(
    float value, const DeviceQuantPoint& point) {
    return static_cast<float>(
        static_cast<double>(value - point.zero_point) *
        static_cast<double>(point.scale));
}

__device__ __forceinline__ float applyRescaleDevice(
    float value, const DeviceRescale& encoded) {
    return cuda_detail::applyEncodedRescaleCore(
        value, encoded.kind == DeviceRescaleKind::MShift,
...
__device__ __forceinline__ float realActivationDevice(
    ...
    const float result = cuda_detail::realActivationCore(
        ...
        params.output.scale,
        static_cast<std::int32_t>(params.output.zero_point),
...
__global__ void quantizeInitialStateKernel(
    const float* source, float* destination, std::size_t count,
    DeviceQuantPoint point) {
    const std::size_t stride =
        static_cast<std::size_t>(gridDim.x) * blockDim.x;
    for (std::size_t index =
         ...
         index < count; index += stride) {
        destination[index] =
            source == nullptr ? point.zero_point
                              : quantizeMasterDevice(source[index], point);
    }
}

template <bool InputWeight>
__global__ void quantizeWeightKernel(
    const float* source, float* destination, std::size_t count,
    ...
    for (std::size_t index =
        ...
        const DeviceQuantPoint point =
            InputWeight ? channel_params[channel].weight_ih
                        : channel_params[channel].weight_hh;
        destination[index] = quantizeMasterDevice(source[index], point);
    }
}

template <bool InputBias>
__global__ void quantizeBiasKernel(
    const float* source, float* destination, std::size_t channels,
    const DeviceLinearChannelParams* channel_params) {
    const std::size_t stride =
        static_cast<std::size_t>(gridDim.x) * blockDim.x;
    for (std::size_t channel =
        ...
        const DeviceQuantPoint point =
            InputBias ? channel_params[channel].bias_ih
                      : channel_params[channel].bias_hh;
        destination[channel] =
            quantizeMasterDevice(source[channel], point);
    }
}

__global__ void weightSumKernel(const float* weights, float* sums,
                                std::size_t channels,
                                std::size_t reduction) {
    const std::size_t stride =
        static_cast<std::size_t>(gridDim.x) * blockDim.x;
...
__global__ void fusedLstmPointwiseKernel(
    std::size_t batch, std::size_t hidden, std::size_t time,
        ...
        for (int gate = 0; gate < 4; ++gate) {
            ...
            if (hidden_index < hidden && scalar_params.bias_enabled != 0) {
                const std::size_t channel =
                    static_cast<std::size_t>(gate) * hidden +
                    hidden_index;
...
#pragma unroll
        for (int gate = 0; gate < 4; ++gate) {
            ...
            const std::size_t linear_index =
                batch_index * channels + channel;
            const DeviceLinearChannelParams& linear =
                channel_params[channel];

            float input_accumulator =
                input_gemm[linear_index] -
                weight_ih_sums[channel] * scalar_params.input.zero_point;
            float recurrent_accumulator =
                recurrent_gemm[linear_index] -
            ...
            if (scalar_params.bias_enabled != 0) {
                input_accumulator += applyRescaleDevice(
                    shared_bias_ih[threadIdx.x][gate] -
                        linear.bias_ih.zero_point,
                    linear.bias_ih_to_accumulator);
                recurrent_accumulator += applyRescaleDevice(
                    shared_bias_hh[threadIdx.x][gate] -
                        linear.bias_hh.zero_point,
            ...
            const float gate_input = clampDevice(
                applyRescaleDevice(
                    input_linear - linear.input_linear.zero_point,
            ...
            const std::size_t checkpoint_index =
                (time * batch + batch_index) * channels + channel;
            if (checkpoints.weight_ih_linear != nullptr) {
                checkpoints.weight_ih_linear[checkpoint_index] =
                    input_linear;
            }
            if (checkpoints.weight_hh_linear != nullptr) {
                checkpoints.weight_hh_linear[checkpoint_index] =
                    recurrent_linear;
            }
        ...
        const std::size_t state_index =
            batch_index * hidden + hidden_index;
        ...
        const auto hidden_result =
            cuda_detail::computeQuantizedHiddenFpEncodedCore(
                gate_outputs[3],
                scalar_params.gates[3].output.zero_point, cell_tanh,
                scalar_params.cell_tanh.zero_point,
                scalar_params.hidden_product_to_output.kind ==
                    DeviceRescaleKind::MShift,
                scalar_params.hidden_product_to_output.multiplier,
                scalar_params.hidden_product_to_output.shift,
        ...
        const std::size_t output_index =
            (time * batch + batch_index) * hidden + hidden_index;
        output[output_index] =
            dequantizeDevice(next_hidden, scalar_params.hidden);
...
__global__ void dequantizeFinalStatesKernel(
    const float* hidden_state, const float* cell_state,
    float* final_hidden, float* final_cell, std::size_t count,
    DeviceQuantPoint hidden_point, DeviceQuantPoint cell_point) {
    const std::size_t stride =
        static_cast<std::size_t>(gridDim.x) * blockDim.x;
    for (std::size_t index =
             static_cast<std::size_t>(blockIdx.x) * blockDim.x +
         ...
         index < count; index += stride) {
        final_hidden[index] =
            dequantizeDevice(hidden_state[index], hidden_point);
        final_cell[index] =
            dequantizeDevice(cell_state[index], cell_point);
    }
}

void checkKernel(const char* name) {
    checkCuda(cudaGetLastError(), name);
}
...
void lstmForwardQuantizedFpCuda(
    ...
    const LstmOperatorQuantConfig& resolved_config,
    const LstmQuantParams& quant_params,
    ...
    if ((initial_hidden == nullptr) != (initial_cell == nullptr)) {
        throw std::invalid_argument(
            "initial_hidden/initial_cell 必须同时提供或省略");
    }
    if (master_weights.weight_ih == nullptr ||
        master_weights.weight_hh == nullptr || input == nullptr ||
        output == nullptr || final_hidden == nullptr ||
        final_cell == nullptr) {
        throw std::invalid_argument("CUDA 量化前向必需指针不能为空");
    }
    if (quant_params.bias_enabled &&
        (master_weights.bias_ih == nullptr ||
         master_weights.bias_hh == nullptr)) {
    ...
    if ((workspace.data == nullptr) != (workspace.bytes == 0)) {
        throw std::invalid_argument(
            "workspace pointer/bytes 必须同时提供或同时省略");
    }
    ...
    if (context.device_execution_params != nullptr &&
        context.device_execution_params_bytes <
    ...
    const int batch_size = cuda_detail::checkedCublasInt(
        shape.batch_size, "batch_size");
    const int input_size = cuda_detail::checkedCublasInt(
        shape.input_size, "input_size");
    const int hidden_size = cuda_detail::checkedCublasInt(
    ...
    const std::size_t sequence_batch_size =
        static_cast<std::size_t>(sequence_batch);
    const std::size_t states = checkedMul(batch, hidden, "B*H");
    const std::size_t sequence_states =
        checkedMul(checkedMul(steps, batch, "T*B"), hidden, "T*B*H");
    const std::size_t sequence_linears =
        checkedMul(checkedMul(steps, batch, "T*B"), channels,
                   "T*B*4H");
    ...
    validateDeviceSpan(input,
                       floatBytes(
                           checkedMul(sequence_batch_size, input_width,
                                      "T*B*I"),
                           "input"),
                       current_device, "input");
    validateDeviceSpan(
        master_weights.weight_ih,
        floatBytes(checkedMul(channels, input_width, "4H*I"),
                   "weight_ih"),
        current_device, "weight_ih");
    validateDeviceSpan(
        master_weights.weight_hh,
        floatBytes(checkedMul(channels, hidden, "4H*H"),
                   "weight_hh"),
        current_device, "weight_hh");
    if (quant_params.bias_enabled) {
        validateDeviceSpan(master_weights.bias_ih,
                           floatBytes(channels, "bias_ih"),
                           current_device, "bias_ih");
        validateDeviceSpan(master_weights.bias_hh,
                           floatBytes(channels, "bias_hh"),
                           current_device, "bias_hh");
    }
    if (initial_hidden != nullptr) {
        validateDeviceSpan(initial_hidden, floatBytes(states, "h_0"),
                           current_device, "initial_hidden");
        validateDeviceSpan(initial_cell, floatBytes(states, "c_0"),
                           current_device, "initial_cell");
    }
    validateDeviceSpan(output, floatBytes(sequence_states, "output"),
                       current_device, "output");
    validateDeviceSpan(final_hidden, floatBytes(states, "h_n"),
                       current_device, "final_hidden");
    validateDeviceSpan(final_cell, floatBytes(states, "c_n"),
                       current_device, "final_cell");
    if (workspace.data != nullptr) {
        validateDeviceSpan(workspace.data, layout.breakdown.total_bytes,
                           current_device, "workspace");
        if ((reinterpret_cast<std::uintptr_t>(workspace.data) %
             kWorkspaceAlignment) != 0) {
            throw std::invalid_argument(
                "caller workspace 必须至少 256-byte 对齐");
        }
    }
    if (context.device_execution_params != nullptr) {
        validateDeviceSpan(
    ...
    if (checkpoints != nullptr) {
        ...
        const std::size_t linear_bytes =
            floatBytes(sequence_linears, "linear checkpoint");
        const std::size_t state_bytes =
            floatBytes(sequence_states, "state checkpoint");
        validateOptionalDeviceSpan(checkpoints->weight_ih_linear,
                                   linear_bytes, current_device,
                                   "checkpoint weight_ih_linear");
        validateOptionalDeviceSpan(checkpoints->weight_hh_linear,
                                   linear_bytes, current_device,
                                   "checkpoint weight_hh_linear");
        validateOptionalDeviceSpan(checkpoints->gate_inputs,
                                   linear_bytes, current_device,
                                   "checkpoint gate_inputs");
        validateOptionalDeviceSpan(checkpoints->gate_outputs,
                                   linear_bytes, current_device,
                                   "checkpoint gate_outputs");
        validateOptionalDeviceSpan(checkpoints->cell_states, state_bytes,
                                   current_device,
                                   "checkpoint cell_states");
        validateOptionalDeviceSpan(checkpoints->cell_tanh_outputs,
    ...
    checkCuda(cudaGetDeviceProperties(&properties, current_device),
              "cudaGetDeviceProperties");
    const dim3 pointwise_block(32, 8);
    const unsigned int grid_x = static_cast<unsigned int>(
        (hidden + pointwise_block.x - 1) / pointwise_block.x);
    const unsigned int requested_grid_y = static_cast<unsigned int>(
        std::min<std::size_t>(
            (batch + pointwise_block.y - 1) / pointwise_block.y,
    ...
    const std::vector<DeviceLinearChannelParams> host_channel_params =
        packLinearParameters(execution_params,
    ...
    const DeviceScalarParams scalar_params =
        packScalarParameters(execution_params,
                             quant_params.bias_enabled);

    const bool uses_internal_workspace = workspace.data == nullptr;
    cuda_detail::OwnedMultiStreamWorkspace owned_workspace(
        uses_internal_workspace ? layout.breakdown.total_bytes : 0,
        context.streams[0], context.streams[1], context.events[0]);
    ...
    float* quantized_hidden =
        region<float>(workspace.data, layout.quantized_state);
    float* quantized_cell = quantized_hidden + states;
    float* input_linear =
        region<float>(workspace.data, layout.input_linear);
    ...
    auto* device_channel_params =
        context.device_execution_params != nullptr
            ? static_cast<DeviceLinearChannelParams*>(
                  context.device_execution_params)
            : region<DeviceLinearChannelParams>(
                  workspace.data, layout.device_parameters);

    const cuda_detail::CublasMathMode shared_math_mode =
        math_mode == LstmQuantizedFpCudaMathMode::Pedantic
            ? cuda_detail::CublasMathMode::Pedantic
            : cuda_detail::CublasMathMode::Tf32;
    cuda_detail::ScopedCublasSettings recurrent_settings(
    ...
    if (!parameters_cached) {
        ...
        checkCuda(
            cudaMemcpyAsync(
                device_channel_params, host_channel_params.data(),
                layout.breakdown.device_parameter_bytes,
                cudaMemcpyHostToDevice, context.streams[0]),
            "copy flattened 4H device parameters");
        checkCuda(cudaStreamSynchronize(context.streams[0]),
                  "wait flattened 4H device parameters");
        if (context.device_execution_params != nullptr) {
            context.cached_execution_signature = signature;
        }
    }
    ...
    quantizeInitialStateKernel
        <<<oneDimensionalBlocks(states), kThreads, 0,
           context.streams[0]>>>(
            initial_hidden, quantized_hidden, states,
            scalar_params.hidden);
    ...
    for (int time = 0; time < sequence_length; ++time) {
        cuda_detail::runGemm(context.handles[0], batch_size, channels_int,
                             hidden_size, quantized_hidden,
                             quantized_weight_hh, recurrent_linear);
        if (time == 0) {
            checkCuda(cudaStreamWaitEvent(context.streams[0],
                                          context.events[0], 0),
                      "pointwise stream wait all-time input GEMM");
        }
        fusedLstmPointwiseKernel
            <<<pointwise_grid, pointwise_block, 0,
               context.streams[0]>>>(
                batch, hidden, static_cast<std::size_t>(time),
                input_linear +
                    static_cast<std::size_t>(time) * batch * channels,
                recurrent_linear, quantized_bias_ih, quantized_bias_hh,
                weight_ih_sums, weight_hh_sums,
                device_channel_params, scalar_params,
                quantized_hidden, quantized_cell, output,
                device_checkpoints);
        checkKernel("fused LSTM pointwise kernel");
    }
    if (timing_events != nullptr) {
        checkCuda(cudaEventRecord(timing_events->core_complete,
                                  context.streams[0]),
                  "record quantized core complete");
    }

    dequantizeFinalStatesKernel
        <<<oneDimensionalBlocks(states), kThreads, 0,
           context.streams[0]>>>(
            quantized_hidden, quantized_cell, final_hidden, final_cell,
            states, scalar_params.hidden, scalar_params.cell);
    checkKernel("dequantize final states kernel");
    if (timing_events != nullptr) {
        checkCuda(cudaEventRecord(timing_events->complete,
                                  context.streams[0]),
