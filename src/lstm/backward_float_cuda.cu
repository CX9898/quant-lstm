...
std::int64_t checkedProduct(std::initializer_list<std::int64_t> factors,
    ...
    for (const std::int64_t factor : factors) {
        if (factor <= 0 ||
            result > std::numeric_limits<std::int64_t>::max() / factor) {
...
void validateBackwardArguments(
    ...
    checkedProduct({shape.sequence_length, shape.batch_size, shape.input_size,
                    shape.hidden_size},
                   "LSTM backward shape");
    if (handle == nullptr) {
        throw std::invalid_argument("cuBLAS handle 不能为空");
    }
    if (weights.weight_ih == nullptr || weights.weight_hh == nullptr ||
        input == nullptr || trace.gate_outputs == nullptr ||
        trace.cell_states == nullptr || trace.cell_tanh_outputs == nullptr ||
        trace.hidden_outputs == nullptr || grad_output == nullptr ||
        grad_final_hidden == nullptr || grad_final_cell == nullptr ||
...
void runRightGemm(cublasHandle_t handle, int rows, int columns,
                  int reduction, const float* left, const float* right,
                  float* output) {
    constexpr float alpha = 1.0F;
    constexpr float beta = 0.0F;
    cuda_detail::checkCublas(
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, columns, rows,
                    reduction, &alpha, right, columns, left, reduction,
                    &beta, output, columns),
        "cublasSgemm backward input");
}

// row-major C[K,N] = A[M,K]^T * B[M,N].
void runWeightGradientGemm(cublasHandle_t handle, int rows,
                           int columns, int reduction,
                           const float* grad_gate_inputs,
                           const float* activations, float* output) {
    constexpr float alpha = 1.0F;
    constexpr float beta = 0.0F;
    cuda_detail::checkCublas(
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, columns, reduction,
                    rows, &alpha, activations, columns, grad_gate_inputs,
                    reduction, &beta, output, columns),
...
void lstmBackwardFloatCuda(
    ...
    const int batch =
        cuda_detail::checkedCublasInt(shape.batch_size, "batch_size");
    const int input_size =
        cuda_detail::checkedCublasInt(shape.input_size, "input_size");
    const int hidden =
        cuda_detail::checkedCublasInt(shape.hidden_size, "hidden_size");
    ...
    const std::int64_t gate_elements =
    ...
    if (initial_hidden == nullptr) {
        cuda_detail::checkCuda(
            cudaMemsetAsync(previous_hidden, 0,
                            static_cast<std::size_t>(state_elements) *
                                sizeof(float),
                            stream),
    ...
    if (steps > 1) {
        cuda_detail::checkCuda(
            cudaMemcpyAsync(previous_hidden + state_elements,
    ...
    cuda_detail::checkCuda(
        cudaMemcpyAsync(gradients.initial_hidden, grad_final_hidden,
                        static_cast<std::size_t>(state_elements) * sizeof(float),
                        cudaMemcpyDeviceToDevice, stream),
        "cudaMemcpyAsync grad h_n");
    cuda_detail::checkCuda(
        cudaMemcpyAsync(gradients.initial_cell, grad_final_cell,
                        static_cast<std::size_t>(state_elements) * sizeof(float),
                        cudaMemcpyDeviceToDevice, stream),
        "cudaMemcpyAsync grad c_n");

    cuda_detail::ScopedCublasSettings settings(
        handle, stream, cuda_detail::CublasMathMode::Pedantic);
    constexpr int threads = 256;
    constexpr std::int64_t max_blocks = 65535;
    const auto state_blocks = static_cast<unsigned int>(
    ...
    for (int time = steps - 1; time >= 0; --time) {
        ...
        const std::int64_t gate_offset =
            static_cast<std::int64_t>(time) * batch * gates;
    ...
    const auto bias_blocks = static_cast<unsigned int>(
        std::min<std::int64_t>(max_blocks,
