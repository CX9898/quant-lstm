    ...
    for (float value : values) {
        if (!std::isfinite(value)) {
            throw std::invalid_argument(
                "Reference Golden 不允许非有限 float32");
        }
        ...
        for (int precision = 1; precision <= 9; ++precision) {
            ...
            float parsed = 0.0F;
            const auto decoded =
                std::from_chars(buffer.data(), encoded.ptr, parsed,
                                std::chars_format::general);
            if (decoded.ec == std::errc{} && decoded.ptr == encoded.ptr &&
                parsed == value && std::signbit(parsed) == std::signbit(value)) {
...
Json floatTensor(const std::vector<std::size_t>& shape,
...
quant_lstm::LstmQuantParams makeParams(
    ...
    for (std::size_t index = 0; index < quant_lstm::kQuantOperatorCount;
        ...
        const std::size_t count =
            quant_lstm::isParameterOperator(id) ? channels : 1;
...
PreparedCase prepare(const CaseDefinition& definition) {
    ...
    result.quant_params =
        makeParams(result.config, definition.shape.hidden_size);
    ...
    result.input = quantizeTensor(definition.input, QuantOperator::Input,
                                  result.config, result.quant_params);
    result.initial_hidden =
        quantizeTensor(initial_hidden, QuantOperator::Output, result.config,
                       result.quant_params);
    result.initial_cell =
        quantizeTensor(initial_cell, QuantOperator::CellState, result.config,
                       result.quant_params);
    result.weight_ih =
        quantizeTensor(weight_ih, QuantOperator::WeightInputHidden,
                       result.config, result.quant_params, 2);
    result.weight_hh =
        quantizeTensor(weight_hh, QuantOperator::WeightHiddenHidden,
                       result.config, result.quant_params, 2);
    result.bias_ih =
        quantizeTensor(bias_ih, QuantOperator::BiasInputHidden, result.config,
                       result.quant_params, 1);
    result.bias_hh =
        quantizeTensor(bias_hh, QuantOperator::BiasHiddenHidden, result.config,
                       result.quant_params, 1);
    return result;
}
...
Json serializedInputs(const PreparedCase& source) {
    ...
    inputs["scale_mode"] = scalarTensor(
        "string", std::string(quant_lstm::scaleModeName(
                      source.config.scale_mode)));
    inputs["bias_enabled"] =
        scalarTensor("bool", source.quant_params.bias_enabled);
    ...
    for (std::size_t index = 0; index < quant_lstm::kQuantOperatorCount;
        ...
        granularities.emplace_back(
            quant_lstm::granularityName(params.source_granularity));
        group_counts.push_back(
            static_cast<std::int32_t>(params.group_diagnostics.size()));
    ...
    inputs["operator_names"] =
        tensor("string", {quant_lstm::kQuantOperatorCount}, operator_names);
    inputs["operator_bitwidth"] =
        tensor("uint8", {quant_lstm::kQuantOperatorCount}, bitwidths);
    inputs["operator_is_unsigned"] =
        tensor("bool", {quant_lstm::kQuantOperatorCount}, is_unsigned);
    ...
    inputs["quant_param_scales"] =
        floatTensor({scales.size()}, scales);
    inputs["quant_param_zero_points"] =
        tensor("int32", {zero_points.size()}, zero_points);
    ...
    inputs["initial_hidden"] =
        tensor("int32",
               {static_cast<std::size_t>(shape.batch_size), hidden},
    ...
    inputs["initial_cell"] =
        tensor("int32",
               ...
               source.initial_cell);
    inputs["weight_ih"] =
        tensor("int32", {channels, static_cast<std::size_t>(shape.input_size)},
               source.weight_ih);
    inputs["weight_hh"] =
        tensor("int32", {channels, hidden}, source.weight_hh);
...
std::int32_t checkedGridValue(float value) {
    if (!std::isfinite(value) || std::nearbyint(value) != value ||
        value < static_cast<float>(INT32_MIN) ||
        value > static_cast<float>(INT32_MAX)) {
        throw std::runtime_error("CPU FP32 reference 产生了非 int32 q 网格值");
    }
    return static_cast<std::int32_t>(value);
}

std::vector<std::int32_t> checkedGridValues(
    const std::vector<float>& values) {
...
std::vector<std::int64_t> checkedWideIntegers(
    const std::vector<float>& values) {
    std::vector<std::int64_t> result;
    result.reserve(values.size());
    for (float value : values) {
        if (!std::isfinite(value) || std::nearbyint(value) != value ||
            value < static_cast<float>(INT64_MIN) ||
            value > static_cast<float>(INT64_MAX)) {
            throw std::runtime_error(
                "CPU FP32 reference 产生了非 int64 整数宽值");
        }
...
void addCheckpointSet(Json& checkpoints, const std::vector<T>& linear_ih,
                      ...
                      const std::vector<T>& cell_states,
                      const std::vector<T>& cell_tanh_outputs,
                      const std::vector<T>& hidden_outputs,
                      const std::vector<T>& final_hidden,
                      const std::vector<T>& final_cell, std::size_t steps,
                      std::size_t batch, std::size_t hidden) {
    const auto qTensor = [&](const std::vector<T>& values,
    ...
    checkpoints["weight_ih_linear"] =
        qTensor(linear_ih, {steps, batch, 4, hidden});
    checkpoints["weight_hh_linear"] =
        qTensor(linear_hh, {steps, batch, 4, hidden});
    ...
    for (const auto& [name, gate] : gates) {
        checkpoints[std::string(name) + "_gate_input"] =
            qTensor(gateSlice(gate_inputs, steps, batch, hidden, gate),
                    gate_shape);
        checkpoints[std::string(name) + "_gate_output"] =
            qTensor(gateSlice(gate_outputs, steps, batch, hidden, gate),
                    gate_shape);
    }
    checkpoints["cell"] = qTensor(cell_states, {steps, batch, hidden});
    checkpoints["cell_tanh"] =
        qTensor(cell_tanh_outputs, {steps, batch, hidden});
    checkpoints["output"] =
        qTensor(hidden_outputs, {steps, batch, hidden});
...
Json intExpected(const PreparedCase& source) {
    ...
    quant_lstm::lstmForwardInt32CpuReference(
        shape,
        {source.weight_ih.data(), source.weight_hh.data(),
         source.bias_ih.data(), source.bias_hh.data()},
        source.input.data(), source.initial_hidden.data(),
        source.initial_cell.data(), source.config, source.quant_params,
        source.execution_params, output.data(), final_hidden.data(),
        final_cell.data(), &trace);

    Json checkpoints;
    addCheckpointSet(checkpoints, trace.weight_ih_linear,
                     trace.weight_hh_linear, trace.gate_inputs,
                     trace.gate_outputs, trace.cell_states,
                     trace.cell_tanh_outputs, trace.hidden_outputs,
                     final_hidden, final_cell, steps, batch, hidden);
    ...
    scaled_contributions.reserve(
        2 * trace.scaled_forget_contributions.size());
    for (std::size_t index = 0; index < trace.p_forget.size(); ++index) {
        ...
        scaled_contributions.push_back(
            trace.scaled_forget_contributions[index]);
        scaled_contributions.push_back(
            trace.scaled_input_contributions[index]);
    }
    Json diagnostics;
    diagnostics["cell_products"] =
        tensor("int64", {steps, batch, hidden, 2}, cell_products);
...
Json fpExpected(const PreparedCase& source) {
    ...
    quant_lstm::lstmForwardQuantizedFpCpuReference(
        ...
        input.data(), initial_hidden.data(), initial_cell.data(), source.config,
        source.quant_params, source.execution_params, output.data(),
        final_hidden.data(), final_cell.data(), &trace);

    Json checkpoints;
    addCheckpointSet(
        checkpoints, checkedGridValues(trace.weight_ih_linear),
        checkedGridValues(trace.weight_hh_linear),
        checkedGridValues(trace.gate_inputs),
        checkedGridValues(trace.gate_outputs),
        checkedGridValues(trace.cell_states),
        checkedGridValues(trace.cell_tanh_outputs),
        checkedGridValues(trace.hidden_outputs),
        checkedGridValues(final_hidden), checkedGridValues(final_cell), steps,
        batch, hidden);
    ...
    Json diagnostics;
    diagnostics["cell_products"] =
        tensor("int64", {steps, batch, hidden, 2},
               checkedWideIntegers(cell_products));
    diagnostics["scaled_cell_contributions"] =
        floatTensor({steps, batch, hidden, 2}, scaled_contributions);
    diagnostics["cell_pre_round_sum"] =
        floatTensor({steps, batch, hidden}, trace.cell_wide_sums);
    diagnostics["hidden_raw_product"] =
        tensor("int64", {steps, batch, hidden},
               checkedWideIntegers(trace.hidden_products));
    return Json{{"checkpoints", checkpoints}, {"diagnostics", diagnostics}};
}
...
Json generateDocument(const PreparedCase& source, bool fp_carrier) {
    return Json{
        {"schema_version", 1},
        {"case_id",
         source.definition.id_prefix +
             (fp_carrier ? "_cpu_fp32" : "_cpu_int32")},
...
void writeDocument(const std::filesystem::path& output_directory,
                   const Json& document) {
    ...
    const std::filesystem::path output_path =
        kind_directory /
        (document.at("case_id").get<std::string>() + ".json");
...
int main(int argc, char** argv) {
    try {
        if (argc != 2) {
            throw std::invalid_argument(
                "usage: golden_reference_generator <golden-spec-dir>");
        }
        const std::filesystem::path output_directory(argv[1]);
        const std::array<CaseDefinition, 2> cases{{
            {"reference_cell_t1_b1_i2_h2", "cell", {1, 1, 2, 2},
             quant_lstm::quantization::ScaleMode::Affine,
             {-0.75F, 0.25F}},
            {"reference_recurrent_t3_b1_i2_h2", "recurrent", {3, 1, 2, 2},
        ...
        for (const auto& definition : cases) {
            ...
            writeDocument(output_directory,
                          generateDocument(prepared, false));
            writeDocument(output_directory,
                          generateDocument(prepared, true));
        }
